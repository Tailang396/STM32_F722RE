...
void bsp_uart_send_dma(bsp_uart_e e, uint8_t *s, uint16_t l) {
    BSP_ASSERT(e == E_UART_CDC || handle[e]);
    if(e == E_UART_CDC) {
        // USB CDC
#ifdef USE_USB_CDC
...
void bsp_uart_printf(bsp_uart_e e, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    uint16_t len = vsnprintf((char *)uart_tx_buf, UART_BUFFER_SIZE, fmt, ap);
    va_end(ap);
    bsp_uart_send(e, uart_tx_buf, len);
}

void bsp_uart_printf_dma(bsp_uart_e e, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    uint16_t len = vsnprintf((char *)uart_tx_buf, UART_BUFFER_SIZE, fmt, ap);
    va_end(ap);
    bsp_uart_send_dma(e, uart_tx_buf, len);
}
