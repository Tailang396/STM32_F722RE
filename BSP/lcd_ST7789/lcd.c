 ...
 */
static const SPI_HandleTypeDef *lcd_spi = &hspi3;
...
#else
	HAL_SPI_Transmit((SPI_HandleTypeDef *)lcd_spi, &dat, 1, 10);
#endif
  LCD_CS_Set();
}
/**
...
/**
************************************************************************
* @brief:      	LCD_ShowFloatNum: 在LCD上显示格式化的浮点数，支持负数,四舍五入
* @param:      	x - x坐标
*              	y - y坐标
*              	num - 要显示的浮点数
*              	len - 整数位数
...
{
	uint8_t t, sizex = sizey / 2;
	uint8_t sign_width  = 1;
	// 处理负数（仅显示符号，不计入整数位数）
	if (num < 0) {
		num = -num;
	...
	x += sizex;
	uint32_t num_int = (uint32_t)(num * mypow(10, decimal) + 0.5);
	// 计算总显示宽度（包含负号、整数、小数点、小数部分）
	uint16_t total_chars = 1 + len + (decimal > 0 ? 1 : 0) + decimal;
	uint16_t fill_width = total_chars * sizex;
	uint16_t fill_height = sizey;
	LCD_Fill(x, y, x - sizex + fill_width, y + fill_height, bc);
	// 显示整数部分（正确位数：len）
	for (t = 0; t < len; t++) {
		uint8_t digit = (num_int / mypow(10, len + decimal - t - 1)) % 10;
		LCD_ShowChar(x + t * sizex, y, '0' + digit, fc, bc, sizey, 0);
	}
	// 显示小数点和小数部分
	if (decimal > 0) {
		LCD_ShowChar(x + len * sizex, y, '.', fc, bc, sizey, 0);
		for (t = 0; t < decimal; t++) {
			uint8_t digit = (num_int / mypow(10, decimal - t - 1)) % 10;
			LCD_ShowChar(x + (len + 1 + t) * sizex, y, '0' + digit, fc, bc, sizey, 0);
		}
	}
}
...
/**
************************************************************************
* @brief:      	LCD_ShowFloatPositiveNum: 在LCD上显示格式化的浮点数，不支持负数，四舍五入
* @param:      	x - x坐标
*              	y - y坐标
*              	num - 要显示的浮点数
*              	len - 整数位数
...
void LCD_ShowFloatPositiveNum(uint16_t x, uint16_t y, float num, uint8_t len, uint8_t decimal, uint16_t fc, uint16_t bc, uint8_t sizey)
{
	uint8_t t, sizex = sizey / 2;
	// 负数处理：强制显示为整数部分 len 位的 "000"（无小数点）
	if (num < 0) {
		num = 0;
		decimal = 0;  // 强制小数部分为0，避免显示小数点和小数
	}
	// 四舍五入转换整数
	uint32_t num_int = (uint32_t)(num * mypow(10, decimal) + 0.5);
	// 计算总显示宽度（整数部分 + 小数点 + 小数部分）
	uint16_t total_chars = len + (decimal > 0 ? 1 : 0) + decimal;
	uint16_t fill_width = total_chars * sizex;
	uint16_t fill_height = sizey;
	// 清除显示区域
	LCD_Fill(x, y, x + fill_width, y + fill_height, bc);
	// 显示整数部分（强制 len 位，不足补零）
	for (t = 0; t < len; t++) {
		uint8_t digit = (num_int / mypow(10, len + decimal - t - 1)) % 10;
		LCD_ShowChar(x + t * sizex, y, '0' + digit, fc, bc, sizey, 0);
	}
	// 显示小数点和小数部分（仅当 decimal > 0 时）
	if (decimal > 0) {
		LCD_ShowChar(x + len * sizex, y, '.', fc, bc, sizey, 0);
		for (t = 0; t < decimal; t++) {
			uint8_t digit = (num_int / mypow(10, decimal - t - 1)) % 10;
...
*              	pic[] - 图片数组地址
* @retval:     	void
* @details:     说明：Image2LCD 3.2版本软件生成代码 4.0版本不行
	            设置：  扫描模式：水平扫描
			    输出灰度：16位真彩色
			    最大宽度和高度： 图片分辨率
			    高位在前 √ 打勾 其它的均不要√
...
/**
************************************************************************
* @brief:      	LCD_Init: LCD初始化函数,延迟小于500ms，不要在OS中调用
* @param:      	void
