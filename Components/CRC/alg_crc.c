...
crc_status_t crc16_verify(const uint8_t *buf, uint32_t len, uint16_t crc) {
    crc_status_t ret = CRC_NO_ERR;
    if (crc != crc16_calc(buf, len))
        ret = CRC_VERIFY_ERR;
    return ret;
}
...
/**
 * @brief 计算CRC8校验值
 * @param buf 指向待计算校验值的数据缓冲区的指针
 * @param len 数据缓冲区的长度（字节数）
 * @param InitCRC 初始CRC值，用于初始化校验计算
 * @param poly CRC8多项式类型，指定使用的CRC算法
 * @return 计算得到的CRC8校验值
 */
uint8_t crc8_calc(const uint8_t *buf, uint32_t len, uint8_t InitCRC, crc8_polynomial_e poly) {
    uint8_t crc = InitCRC;
    while (len--) {
        crc = CRC8_TAB[poly][crc ^ *buf++];
    }
    return crc;
}
...
/**
 * @brief 验证CRC8校验码是否正确
 * @param buf 指向待验证数据缓冲区的指针
 * @param len 数据缓冲区的长度（字节数）
 * @param InitCRC CRC计算的初始值
 * @param poly CRC8多项式选择枚举值
 * @param crc 待验证的CRC8校验码
 * @return crc_status_t 返回验证结果状态
 *         - CRC_NO_ERR: 校验通过，CRC值匹配
 *         - CRC_VERIFY_ERR: 校验失败，CRC值不匹配
 */
crc_status_t crc8_verify(const uint8_t *buf, uint32_t len, uint8_t InitCRC, crc8_polynomial_e poly, uint8_t crc) {
    crc_status_t ret = CRC_NO_ERR;
    // 计算缓冲区数据的CRC8值并与提供的CRC值进行比较
    if (crc != crc8_calc(buf, len, InitCRC, poly))
        ret = CRC_VERIFY_ERR;
    return ret;
}
